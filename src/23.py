...
def solve_1(filled_maze):
    ...
    return find_minimum_cost(maze, state)

def find_minimum_cost(maze, state):
...
def find_possible_moves(maze, state, amphipod):
    ...
    non_occupied = [(xn, yn) for xn, yn in non_walls if not is_occupied(state, (xn, yn))]
    return non_occupied
